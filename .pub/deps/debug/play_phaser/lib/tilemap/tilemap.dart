...
class Tilemap {
  ...
  Tilemap(Game game, [String key, num tileWidth = 32, num tileHeight = 32, num width = 10, num height = 10]) {
    ...
    TilemapData data = TilemapParser.parse(this.game, key, tileWidth, tileHeight, width, height);
  ...
  TilemapLayer create(String name, num width, num height, num tileWidth, num tileHeight, [Group group]) {
  ...
  setTileSize(num tileWidth, num tileHeight) {
  ...
  Tileset addTilesetImage(tileset, [String key, num tileWidth, num tileHeight, num tileMargin, num tileSpacing, int gid]) {
    ...
    if (key == null) {
      if (tileset is String) {
        ...
        if (this.game.cache.checkImageKey(key) == null) {
    ...
    if (tileset is String) {
      ...
      if (tileset == null && this.format == Tilemap.TILED_JSON) {
        window.console.warn('Phaser.Tilemap.addTilesetImage: No data found in the JSON matching the tileset name: "' + key + '"');
        return null;
      }
    }

    if (tileset != null && this.tilesets[tileset] != null) {
      ...
      this.tilesets.add(newSet);

      int i = this.tilesets.length - 1;
      num x = tileMargin;
      num y = tileMargin;
  ...
  createFromObjects(String name, num gid, String key, [frame, bool exists, bool autoCull, Group group, Type CustomClass, bool adjustY]) {
    ...
    if (this.objects[name] == null) {
      window.console.warn('Tilemap.createFromObjects: Invalid objectgroup name given: ' + name);
      return;
    }
  ...
  TilemapLayer createLayer(layer, [num width, num height, Group group]) {
    ...
    if (index == null || index > this.layers.length) {
      window.console.warn('Tilemap.createLayer: Invalid layer ID given: ' + index);
      return null;
    }
  ...
  TilemapLayer createBlankLayer(String name, num width, num height, num tileWidth, num tileHeight, [Group group]) {
    ...
    for (int y = 0; y < height; y++) {
      ...
      for (int x = 0; x < width; x++) {
        // row.push(null);
        row.add(new Tile(layer, -1, x, y, tileWidth, tileHeight));
      }
    ...
    TilemapLayer outputLayer = new TilemapLayer(this.game, this, this.layers.length - 1, w, h);
    outputLayer.name = name;

    return group.add(outputLayer);
  ...
  int getIndex(List location, String name) {
  ...
  int getTilesetIndex(String name) {
  ...
  int getObjectIndex(String name) {
    return this.getIndex(this.objects.values, name);
  }
  ...
  setTileIndexCallback(indexes, Function callback, [layer]) {
  ...
  setTileLocationCallback(num x, num y, num width, num height, Function callback, [layer]) {
    layer = this.getLayer(layer);
    ...
    for (var i = 1; i < this._results.length; i++) {
      this._results[i].setCollisionCallback(callback);
    }
  }
  ...
  setCollisionByExclusion(List<int> indexes, [bool collides, layer, bool recalculate]) {
  ...
  setCollisionByIndex(int index, [bool collides, layer, bool recalculate]) {
    ...
    if (collides) {
      ...
      if (i > -1) {
        this.collideIndexes.removeAt(i);
      }
    }

    for (int y = 0; y < this.layers[layer].height; y++) {
      for (int x = 0; x < this.layers[layer].width; x++) {
        var tile = this.layers[layer].data[y][x];

        if (tile != null && tile.index == index) {
          if (collides) {
  ...
  setPreventRecalculate(bool value) {
    ...
    if ((value == false) && (this.preventingRecalculate == true)) {
      ...
      for (var i in this.needToRecalculate.keys) {
        this.calculateFaces(i);
      }
      this.needToRecalculate.clear();
    }
  }
  ...
  Tile getTileAbove(int layer, num x, num y) {
    if (y > 0) {
  ...
  Tile getTileBelow(int layer, num x, num y) {
  ...
  Tile getTileLeft(int layer, num x, num y) {
  ...
  Tile getTileRight(int layer, num x, num y) {
  ...
  bool hasTile(num x, num y, layer) {
  ...
  Tile removeTile(num x, num y, layer) {
  ...
  Tile removeTileWorldXY(num x, num y, num tileWidth, num tileHeight, layer) {
    layer = this.getLayer(layer);
    x = Math.snapToFloor(x, tileWidth) / tileWidth;
    y = Math.snapToFloor(y, tileHeight) / tileHeight;
  ...
  Tile putTileWorldXY(tile, num x, num y, num tileWidth, num tileHeight, [layer]) {
    layer = this.getLayer(layer);
    x = Math.snapToFloor(x, tileWidth) / tileWidth;
    y = Math.snapToFloor(y, tileHeight) / tileHeight;
  ...
  Tile getTile(num x, num y, [layer, bool nonNull]) {
  ...
  Tile getTileWorldXY(num x, num y, [num tileWidth, num tileHeight, layer]) {
  ...
  List<Tile> copy(num x, num y, num width, num height, [layer]) {
    ...
    for (var ty = y; ty < y + height; ty++) {
      for (var tx = x; tx < x + width; tx++) {
        this._results.add(this.layers[layer].data[ty][tx]);
      }
    }
  ...
  paste([num x, num y, List tileblock, layer]) {
  ...
  swap(int tileA, int tileB, num x, num y, num width, num height, [layer]) {
    ...
    this._results.forEach(this.swapHandler);
  ...
  forEach(Function callback, num x, num y, num width, num height, [layer]) {
    ...
    this._results.forEach(callback);
  ...
  replace(int source, int dest, num x, num y, num width, num height, [layer]) {
    ...
    for (int i = 1; i < this._results.length; i++) {
  ...
  random(num x, num y, num width, num height, [layer]) {
  ...
  shuffle(num x, num y, num width, num height, [layer]) {
    ...
    for (int t = 1; t < this._results.length; t++) {
      if (this._results[t].index) {
        indexes.add(this._results[t].index);
      }
    }
  ...
  fill(int index, num x, num y, num width, num height, [layer]) {
    layer = this.getLayer(layer);
    this.copy(x, y, width, height, layer);
  ...
  dump() {
    ...
    for (int y = 0; y < this.layers[this.currentLayer].height; y++) {
      for (int x = 0; x < this.layers[this.currentLayer].width; x++) {
        txt += "%c  ";
